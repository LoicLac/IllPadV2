...
| Bank pads (8 control pads) | `BankPadStore` | Tool PAD ROLE page BANK | `illpad_bpad` / `pads` | New role → règle ABSORBANT/CONTEXTUEL (cf [`tool-pad-role-design.md`](../superpowers/specs/2026-05-23-tool-pad-role-design.md)) ; runtime consumer (BankManager) |
| Scale pads (7 root + 7 mode + 1 chrom) | `ScalePadStore` | Tool PAD ROLE page ARPEG | `illpad_spad` / `pads` | Extend category → Tool PAD ROLE grid + ScaleManager |
| Control pad assignments + 3 global DSP params | `ControlPadStore` v2 | Tool PAD ROLE page CC | `illpad_ctrl` / `pads` | Add entry : new slot + page CC edit ; globals via 'g' key. Consumed at boot via `ControlPadManager::applyStore` |
| Arp pads (1 hold + 4 octave) | `ArpPadStore` | Tool PAD ROLE page ARPEG | `illpad_apad` / `pads` | New ARPEG control pad → Tool PAD ROLE + `main.cpp::handleHoldPad` (à renommer `handleArpPlayStopPad`) + ScaleManager |
| Loop pads (3 controls + 16 slots) | `LoopPadStore` | Tool PAD ROLE page LOOP | `illpad_lpad` / `pads` | Page LOOP gère REC/PL/S/CLR (musical layer) + Slots S00..S15 (shift layer). Writer/UI livrés au plan Tool PAD ROLE. |
...
|---|---|---|---|
| **New mode of play** (loop, beat, sequencer, drum pad) | Clock, BankType, state machine, scheduling, pile, LED, PotMapping, Tool PAD ROLE (probable nouvelle page), NVS | Pressure pipeline, ScaleResolver (unless notes), Battery, BLE detail | ArpEngine + ArpScheduler (complete pattern) + BankManager |
| **New pad role category** (loop controls, macro pads) | Tool PAD ROLE règle ABSORBANT/CONTEXTUEL (cf spec dédiée), new runtime manager, LED state, NVS new Store, main.cpp dispatch | ArpEngine internals, MIDI transport, Clock | ScaleManager (role-based template) + BankManager (pad-detection template) + Tool PAD ROLE |
