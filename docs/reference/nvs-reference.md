...
`NVS_DESCRIPTORS[10]` in `KeyboardData.h` — one entry per Store blob:
...
7 `inline validate*()` functions in `KeyboardData.h`. Called after every `loadBlob` and before every `saveBlob` from external input (future WiFi). Single source of truth for field bounds.
...
|--------|-----------|-----|-------|---------|------|-------|
| `CalDataStore` | `illpad_cal` | `caldata` | 0xBEEF | 5 | 100B | CapacitiveKeyboard (DO NOT MODIFY) |
