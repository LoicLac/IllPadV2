...
```bash
git submodule add git://sigrok.org/libserialport vendor/libserialport
```
...
libserialport uses autotools, easiest is to wrap its build via ExternalProject or build sources directly via CMake:

```cmake
# vendor libserialport — sources built as a static library
...
target_include_directories(libserialport PUBLIC
    ${CMAKE_SOURCE_DIR}/vendor/libserialport
)
target_compile_definitions(libserialport PRIVATE
    LIBSERIALPORT_ATBUILD=1
)
if(APPLE)
...
Note: libserialport requires a `libserialport_internal.h` and the upstream config detection. If direct source build fails, fallback to ExternalProject_Add with autotools (`./autogen.sh && ./configure && make`). See library docs for the actual source list — it varies by version.
...
target_include_directories(libvterm PRIVATE
    ${CMAKE_SOURCE_DIR}/vendor/libvterm/src
)
```
...
Expected: build succeeds. If errors on missing sources, inspect vendor lib structure and adjust source list. Common gotcha: libserialport requires generated `config.h` — provide a minimal one if autotools-based build is skipped.
...
void TransportBar::paint(juce::Graphics& g) {
    ...
    g.setFont(juce::Font("IBM Plex Mono", 28.0f, juce::Font::bold));
    ...
    if (panic) {
        g.setFont(juce::Font("IBM Plex Mono", 16.0f, juce::Font::bold));
        g.setColour(juce::Colours::white);
...
void PotCell::paint(juce::Graphics& g) {
    ...
    g.setFont(juce::Font("IBM Plex Mono", 24.0f, juce::Font::bold));
...
void CurrentBankPanel::paint(juce::Graphics& g) {
    ...
    // big bank number
    g.setColour(juce::Colour(AppLookAndFeel::TEXT_PRIMARY));
    g.setFont(juce::Font("IBM Plex Mono", 48.0f, juce::Font::bold));
...
AllBanksPanel::AllBanksPanel(Model& m) : model(m) {
    ...
    title.setFont(juce::Font("IBM Plex Mono", 12.0f, juce::Font::bold));
...
EventLogPanel::EventLogPanel(Model& m) : model(m) {
    ...
    logEditor.setFont(juce::Font("IBM Plex Mono", 11.0f, juce::Font::plain));
...
void TerminalView::paint(juce::Graphics& g) {
    ...
    auto font = juce::Font("IBM Plex Mono", cellH * 0.85f, juce::Font::plain);
    g.setFont(font);
