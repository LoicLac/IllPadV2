...
public:
    ...
    // ILLPAD fork: system real-time callback (0xF8 Clock, 0xFA Start, 0xFC Stop)
    void setSystemRTCallback(void (*callback)(uint8_t status));

    // ILLPAD fork: send system real-time (0xF8 Clock, 0xFA Start, 0xFC Stop)
    void sendSystemRT(uint8_t status);
...
private:
    ...
    void (*systemRTCallback)(uint8_t) = nullptr;  // ILLPAD fork
