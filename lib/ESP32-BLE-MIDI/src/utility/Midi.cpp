...
void Midi::receivePacket(uint8_t *data, uint8_t size)
{
    ...
    // Helper macro: bytes remaining from ptr to end of buffer
    #define REMAINING() ((int)(size - (ptr - data)))

    while(ptr - data < size) {
        // Timestamp byte (high bit set)
        if(ptr[0] & 0b10000000) {
            currentTimestamp = (currentTimestamp & 0b1111110000000) | (ptr[0] & 0b1111111);
            ptr++;
        }

        if(REMAINING() < 1) break;  // Need at least status byte
        ...
        switch(command) {
            case 0:
                ...
                return;
            case 0b1000: {    // Note off (2 data bytes)
            ...
            case 0b1001: {   // Note on (2 data bytes)
                if(REMAINING() < 2) return;
                uint8_t note = ptr[0];
                uint8_t velocity = ptr[1];
                ptr += 2;
            ...
            case 0b1010: {    // Polyphonic after touch (2 data bytes)
                if(REMAINING() < 2) return;
                uint8_t note = ptr[0];
            ...
            case 0b1011: {    // Control Change (2 data bytes)
                if(REMAINING() < 2) return;
            ...
            case 0b1100: {    // Program Change (1 data byte)
            ...
            case 0b1101: {    // After touch (1 data byte)
                if(REMAINING() < 1) return;
            ...
            case 0b1110: {    // Pitch bend (2 data bytes)
                if(REMAINING() < 2) return;
                uint8_t lsb = ptr[0];
                uint8_t msb = ptr[1];
                ptr += 2;
            ...
            case 0b1111: {
                // ILLPAD fork: handle system real-time (0xF8 Clock, 0xFA Start, 0xFC Stop)
                // System RT messages have no data bytes
                if (runningStatus >= 0xF8) {
                    if (systemRTCallback != nullptr)
                        systemRTCallback(runningStatus);
                    debug.printf("System RT: 0x%02X\n", runningStatus);
                } else {
...
// ILLPAD fork: system real-time callback setter
void Midi::setSystemRTCallback(void (*callback)(uint8_t)) {
    systemRTCallback = callback;
}

// ILLPAD fork: send system real-time (0xF8 Clock, 0xFA Start, 0xFC Stop)
void Midi::sendSystemRT(uint8_t status) {
